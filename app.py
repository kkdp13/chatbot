...
#        replyQueue.append(clarity)
        newpriceusdtext = 'total price is {0:,.2f} USD'.format(newpriceusd)
        newpricetext = 'total price is {0:,.2f} THB'.format(newprice)
        rapaportpricetext = 'the rapaport is {}'.format(price['rapaportprice'])
        currencytext = 'the superrich rate is {0:,.2f}'.format(price['currency'])
